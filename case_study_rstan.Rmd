...
Let's pretend that we are modeling heat diffusion on a rod $x \in [0, L]$ as a partial differential equation
\begin{equation}
...
u(t, x = L) = 1
$$
and the initial heat distribution
\begin{equation}
u(t = 0, x) = 
\begin{cases}
...
\end{cases}
\end{equation}
The parameter $D$ is unknown. Let's pretend that we have measured the concentration $u(t,x)$ of this system at time $t = T$ and try to estimate the diffusion constant $D$.

Let's assume $D_{true} = 0.5$. This is what we are going to try to estimate.
We can easily discretize this using [method of lines](https://en.wikipedia.org/wiki/Method_of_lines) and generate some sample
data in R:
...
my_red <- "#fe8a2f"
plot(x, u_init, type = 'l', main = 'Heat distribution on [0,L]', ylab = 'u(t,x)', xaxt = "n")
lines(x, u, col = my_red)
legend(0.8, 0.4, legend = c("t = 0", "t = T"), col = c("black", my_red),
       lty = c(1,1))
