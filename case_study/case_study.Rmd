...
rng_seed_r <- 999
rng_seed_stan <- 123
set.seed(rng_seed_r) # rng seed for data simulation
```
...
# Compute differences
compute_errors <- function(df, ode_pars) {
  num_draws <- nrow(df)
  mae <- c()
  for (i in 1:num_draws) {
    theta <- as.numeric(df[i, ode_pars])
    mae_i <- check_reliability(theta, atol_high, rtol_high, max_num_steps_high)
    mae <- c(mae, mae_i)
  }
  df$mae <- mae
  return(df)
...
draws <- compute_errors(draws, ode_pars)
...
p1 <- scatter_colored(draws, "beta", "gamma", "mae")
p2 <- scatter_colored(draws, "beta", "phi", "mae")
p3 <- scatter_colored(draws, "gamma", "phi", "mae")
ggarrange(p1, p2, p3)
```
...
qplot(draws$mae, geom = "histogram")
```
...
```{r sir_weights}
compute_log_ratios <- function(df, ode_pars, lh_pars) {
  num_draws <- nrow(df)
  log_lh_low <- rep(0, num_draws)
  log_lh_high <- rep(0, num_draws)
  for (i in seq_len(num_draws)) {
    theta_i <- as.numeric(df[i, ode_pars])
    phi_i <- as.numeric(df[i, lh_pars])
    y_hat_low <- solve_sir(theta_i, rtol_low, atol_low, max_num_steps_low)
    y_hat_high <- solve_sir(theta_i, rtol_high, atol_high, max_num_steps_high)
    log_lh_low[i] <- sum(dnbinom(y, size = phi_i, mu = y_hat_low, log = TRUE))
    log_lh_high[i] <- sum(dnbinom(y, size = phi_i, mu = y_hat_high, log = TRUE))
  }
  draws$log_lh_high <- log_lh_high
  draws$log_lh_low <- log_lh_low
  draws$log_ratio <- log_lh_high - log_lh_low
  return(draws)
}

lh_pars <- "phi"
draws <- compute_log_ratios(draws, ode_pars, lh_pars)

p1 <- scatter_colored(draws, "beta", "gamma", "log_ratio")
p2 <- scatter_colored(draws, "beta", "phi", "log_ratio")
p3 <- scatter_colored(draws, "gamma", "phi", "log_ratio")
ggarrange(p1, p2, p3)
```
...
qplot(draws$log_ratios, geom = "histogram")
```
...
```{r sir_pareto}
r_eff <- loo::relative_eff(x = exp(-draws$log_ratio), draws$chain_id)
psis1 <- loo::psis(draws$log_ratio, r_eff = r_eff)
