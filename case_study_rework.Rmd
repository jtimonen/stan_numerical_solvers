...
Assume we have a Bayesian model that involves an ordinary differential
equation with uncertain parameters. Every log density evaluation we do
requires solving this equation, and with a gradient based MCMC like those in
Stan, gradients too. These computations are often expensive and frequently
become the limiting factors in whatever model they are involved in. Because
of this, it is often attractive to consider cheaper ways to perform the solves.
If an new method is not easily available, then usually there is a tolerance
or discretization parameter than can be adjusted on the solver to speed things
up. The question immediately becomes, is this okay? Has changing the numerical
method affected our inference results?

This is the question that this case study attempts to answer how to check if
a numerical approximation is good enough in the context of an inference. A more
cynical person may have asked, was our original method good to begin with? Or,
are the default tolerances in Stan even suitable for the problem at hand? We
will try to provide answers to these questions as well.

# Motivation

To motivate the rest of this case study, let's look at a simple example to
demonstrate the ways an approximate calculation affects inference.
