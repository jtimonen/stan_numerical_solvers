...
diagnostic.
4. If $\hat{k} > 0.5$, raise precision of $p_{low}$ and go back to step 1.
...
2. Tune the reference method using these draws
...
## 2. Tune the reference method using the draws

Given our posterior draws, we need to a high resolution reference solution
that can accurately solve the system at every point. This does not mean we need
