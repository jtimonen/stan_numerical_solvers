...
and computational cost of the solution. In general, `rtol` is the tolerance on the relative error the solver can make when `y` is far from zero. When `abs(y)` is small (of the order of `atol` or smaller), there is no need to achieve the relative tolerance.
...
ggplot(df, aes) + geom_line() + geom_point() + 
