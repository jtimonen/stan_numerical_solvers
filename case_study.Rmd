...
dt     <- 1e-3    # discretization step in t
...
for (i in seq_len(num_draws)) {
  U_ref[i,] <- stan_be(u_0, dt_ref, dx, T_max, K_draws[i])
}
```
...
for (i in seq_len(num_draws)) {
  u <- U[i,]
  u_ref <- U_ref[i,]
  log_lh[i] <- sum(dnorm(log_y, mean = u, sd = sigma_vec, log = TRUE))
  log_lh_ref[i] <- sum(dnorm(log_y, mean = u_ref, sd = sigma_vec, log = TRUE))
}
```

They look the same
```{r lineplot, fig.width=6, fig.height=4.5}
plot(log_lh, log_lh_ref)
```
