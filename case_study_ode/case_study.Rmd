...
are a model for the rate of change in some quantities $\textbf{y}(t)$, and are used in many fields of science. It is common that we have time-course data $\textbf{y}^{\dagger}_i$, observed at time points $t_i \neq t_0$, and we need to calculate $\textbf{y}(t_i)$ in order to compute the likelihood. Given initial value  $\textbf{y}(t_0) = \textbf{y}_0$, and assuming some smoothness requirements for $f_{\theta}$, there exists a unique solution for $\textbf{y}(t)$, $t \in \mathbb{R}$. This solution is given by the integral formula

\begin{equation}
\textbf{y}(t) = \textbf{y}_0 + \int_{t_0}^t f_{\theta}(\textbf{y}(\tau), \tau) 
\text{d} \tau,
\end{equation}
...
steps need to be taken. Alternatively, methods can estimate
...
parameter) using Stan, the system needs to be solved numerically on each log
posterior probability evaluation. Furthermore, Stan needs to compute gradients
for these solutions. Try as we might, these computations are often expensive
