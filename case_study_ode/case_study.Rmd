...
are a model for the rate of change in some quantities $\textbf{y}(t)$, and are used in many fields of science. It is common that we have time-course data $\textbf{y}^{\dagger}_i$, observed at time points $t_i \neq t_0$, and we need to calculate $\textbf{y}(t_i)$ in order to compute likelihood of the data, given
our model. Given initial value  $\textbf{y}(t_0) = \textbf{y}_0$, and assuming some smoothness requirements for $f_{\theta}$, there exists a unique solution for $\textbf{y}(t)$, $t \in \mathbb{R}$. This solution is given by the integral formula
...
based MCMCM algorithm like Stan does, also \emph{sensitivities}, i.e. 
