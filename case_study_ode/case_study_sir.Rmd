...
sir_likelihood <- "
  for(n in 1:N) {
    I_data[n] ~ neg_binomial_2(x[n][2] + 1e-6, phi);
  }
"
sir_gq <- "
  int y[D, N];
  ...
  for(n in 1:N) {
    for(d in 1:D) {
      y[d,n] = neg_binomial_2_rng(x[n][d] + 1e-6, phi); 
    }
    log_lik += neg_binomial_2_lpmf(I_data[n] | x[n][2] + 1e-6, phi);
  }
"
data_list$D <- 3
```
