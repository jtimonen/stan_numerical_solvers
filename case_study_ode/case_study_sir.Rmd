...
simulate <- function(model, params, t_sim=NULL, RTOL = 1e-6, ATOL = 1e-6,
                     MAX_NUM_STEPS = 1e6) {
...
prior_sim <- simulate(sir_models$sim, prior_draws)
...
simulate_many <- function(model, params, atols, rtols, N, D){
  J1 <- length(atols)
  J2 <- length(rtols)
  S <- niterations(params) * nchains(params)
  TIME <- array(0, dim=c(J1, J2))
  XSIM <- array(0, dim=c(J1, J2, S, N*D))
  for(j1 in 1:J1) {
    for(j2 in 1:J2) {
      sim <- simulate(model, params, ATOL=atols[j1], RTOL=rtols[j2])
...
atols <- TOLS
rtols <- TOLS
sims <- simulate_many(sir_models$sim, prior_draws, atols, rtols, N, 3)
```
...
```{r sir_test_prior_tols_max_error}
# Compute max absolute difference to most accurate solution
max_errors <- function(XSIM) {
  J1 <- dim(XSIM)[1]
  J2 <- dim(XSIM)[2]
  MAE <- array(0, dim=c(J1,J2))
  I_sim_best <- XSIM[1,1,,]
  for(j1 in 1:J1) {
    for(j2 in 1:J2) {
      I_sim <- XSIM[j1,j2,,]
      MAE[j1,j2] <- max(abs(as.vector(I_sim_best)-as.vector(I_sim)))
    }
  }
  return(MAE)
}
