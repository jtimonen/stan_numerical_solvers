...
mod <- cmdstan_model("stan/sir.stan", include_paths = "stan")
mod_sim <- cmdstan_model("stan/sir_simulate.stan", include_paths = "stan")
```
...
# Sample model
fit <- mod$sample(
  data = stan_data, sig_figs = SIG_FIGS, seed = rng_seed_stan,
  refresh = 1000
)

# Get draws of ODE parameters
theta_draws <- fit$draws(c("beta", "gamma"))
mcmc_areas(theta_draws)

# Simulate ODE solutions
sim <- mod_sim$generate_quantities(
  data = stan_data, 
  fitted_params = theta_draws
)
...
i_chain <- 1
i_draw <- 1
S <- as.vector(x_draws[i_draw, i_chain, 1:50])
I <- as.vector(x_draws[i_draw, i_chain, 51:100])
R <- as.vector(x_draws[i_draw, i_chain, 100:150])

plot(t, y)
lines(t, I, col="firebrick")
```
