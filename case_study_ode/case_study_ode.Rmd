...
solver_args_high <- list(
  abs_tol = 1e-10,
  rel_tol = 1e-10,
  max_num_steps=1e9
)
posterior_sim_high <- simulate(sir_models$sim, posterior_draws, data_list,
                               solver_args_high, stan_opts)
```



### Steps 3-4. Compute importance weights and Pareto-$k$

```{r importance_weights}
is <- use_psis(posterior_sim, posterior_sim_high)
print(is)
print(is$diagnostics$pareto_k)
```


### Step 5. Importance resampling
```{r resampling}
posterior_draws_resampled <- posterior::resample_draws(posterior_draws,
